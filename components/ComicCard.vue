...
<template>
    <div class="overflow-hidden rounded-md duration-500 border-2 border-transparent md:hover:border-[#ffeeb0] relative group md:group-hover:shadow-md cursor-pointer"
        @click="(e) => handleClickCard(e, 'detail')">
        <div class="flex gap-1 absolute font-semibold top-0 inset-x-0 z-10 text-xs text-white">
            <span v-if="is_trending" class="bg-rose-500 py-0.5 px-2 rounded-b-sm first:rounded-bl-none">
                Hot
            </span>
            <span v-if="status === 'Completed'" class="bg-sky-500 py-0.5 px-2 rounded-b-sm first:rounded-bl-none">
                End
            </span>
            <span v-if="updated_at?.includes('trước') &&
        ...
        <div class="relative">
            <div :class="`absolute inset-0 flex items-center justify-center text-white bg-gray-200 duration-150 ${isImageLoaded ? 'opacity-0' : 'opacity-100'
                }`">
            ...
            <img :src="thumbnail" :alt="title"
                ...
                loading="lazy" @load="isImageLoaded = true" />
        </div>
        <div
            ...
            <h5
                class="font-bold leading-5 text-lg text-white group-hover:text-[#d9a56f] text-shadow duration-200 line-clamp-2">
            ...
            <template v-if="detail">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-0.5 justify-center gap-x-2 gap-y-1 text-[#d9a56f] text-xs py-1 mt-0.5"
                        v-if="!isHistory">
                    ...
                    <div v-else class="text-gray-300">
                        <p class="text-sm font-semibold flex items-center gap-0.5 mb-1 text-fuchsia-400">
                            <Icon name="ph:read-cv-logo-fill" size="18" />
                            {{ last_reading }}
                        </p>
                        <div class="flex items-center gap-1 text-sm text-white">
                            <button class="bg-sky-500 w-full px-2 py-1 rounded-sm flex justify-center items-center gap-1"
                                @click="(e) => handleClickCard(e, 'continue')">
                                <Icon name="system-uicons:book-text" size="20" />
                                Continue
                            </button>
                            <button class="bg-rose-500 px-2 py-1 rounded-sm" @click="(e) => handleClickCard(e, 'delete')">
