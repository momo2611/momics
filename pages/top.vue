...
const getComics = async (tab: string, page: number = 1, filter = 'all') => {
    try {
        isFetching.value = true;
        const data = await useFetchData(
            `/top/${tab === 'all' ? '' : tab}?page=${page}&status=${filter
            }`
        );
...
watch([currentTab, route, filterValue], async ([newTab, route, newFilter]) => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
    const page = route.query.page || 1;
    await getComics(newTab, Number(page), newFilter);
});
...
<template>
    ...
    <main class="max-w-6xl mx-auto px-3">
        <ul class="flex flex-wrap items-center gap-1.5 mt-5 sm:gap-3">
            <li v-for="route in topRoutes" :key="route.type" :class="`flex items-center gap-1 px-3 py-2 rounded cursor-pointer duration-150 select-none ${route.type === currentTab ? 'bg-[#d9a56f] text-white' : ''
                }`" @click="handleChangeTab(route.type)">
                <Icon :name="route.icon" size="24" />
                {{ route.name }}
            </li>
        </ul>
        <ul class="flex items-center flex-wrap gap-2.5 mb-5 mt-3 font-semibold sm:gap-5">
            <li v-for="item in filterValues" :class="`min-w-[60px] cursor-pointer text-center border px-3 py-1.5 rounded ${item.value === filterValue
