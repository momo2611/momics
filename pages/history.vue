...
<template>
    <Head>
        <Title>{{ `History - Page ${route.query.page ?? 1} | Momics` }}</Title>
        <Meta name="description" content="Đọc truyện tranh online miễn phí" />
    </Head>
    <main class="max-w-6xl mx-auto px-3 min-h-screen">
        <ComicsPagination v-if="comics.length" :is-fetching="isFetching" :comics="comics.slice(
            (currentPage - 1) * COMICS_PER_PAGE,
            currentPage * COMICS_PER_PAGE
        )
            " :total-pages="Math.ceil(comics.length / COMICS_PER_PAGE)" title="Recently Read" icon="mdi:clock-outline"
            :is-history="true" @delete-comic="getHistoryComics" />
        <h3 v-else class="text-2xl mt-8 font-bold text-[#8a8372] text-center">
            No History
        </h3>
    </main>
</template>